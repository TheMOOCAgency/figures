...
import {
  addActiveApiFetch,
  removeActiveApiFetch,
} from "base/redux/actions/Actions";
...
class SingleCourseContent extends Component {
  ...
  fetchCourseData = () => {
    ...
    fetch(apiConfig.coursesDetailed + this.props.courseId + "/", {
  ...
  fetchProgramName = () => {
    fetch(apiConfig.programName + this.props.courseId + "/", {
      credentials: "same-origin",
    })
      .then((response) => response.json())
      .then((json) =>
  ...
  fetchLearnersData = () => {
    this.props.addActiveApiFetch();
    fetch(
      this.state.apiFetchMoreLearnersUrl === null
        ? apiConfig.learnersDetailed +
            "?enrolled_in_course_id=" +
            this.props.courseId
        : this.state.apiFetchMoreLearnersUrl,
      { credentials: "same-origin" }
    )
      .then((response) => response.json())
  ...
  setLearnersData = (results, paginationNext) => {
    const tempLearners = this.state.learnersList.concat(
      Immutable.fromJS(results)
    );
    this.setState(
      {
  ...
  render() {
    return (
      <div className="ef--layout-root">
        ...
        <ReportsBox
          courseId={this.state.courseData.getIn(["course_id"])}
          isProgramCourse={this.state.programName ? true : false}
          learnersEnrolled={this.state.courseData.getIn([
            "learners_enrolled",
            "current_month",
          ])}
        ...
        >
          ...
          <BaseStatCard
            ...
            mainValue={this.state.courseData.getIn([
              "learners_enrolled",
              "current_month",
            ])}
            valueHistory={this.state.courseData.getIn(
              ["learners_enrolled", "history"],
              []
            )}
          ...
          <BaseStatCard
            ...
            mainValue={
              this.state.courseData.getIn([
                "learners_enrolled",
                "current_month",
              ]) - this.state.courseData.get("tma_started")
            }
            secondaryValue={this.state.courseData.getIn([
              "learners_enrolled",
              "current_month",
            ])}
          ...
          <BaseStatCard
            ...
            mainValue={
              this.state.courseData.getIn([
                ...
                ? (this.state.courseData.get("tma_completed") +
                    this.state.courseData.get("tma_partially_completed")) /
                  this.state.courseData.getIn([
                    "learners_enrolled",
                    "current_month",
                  ])
            ...
            <div className={styles["header-title"]}>
              ...
              {this.state.courseData.getIn([
                ...
                <a
                  ...
                  href={
                    "/tma_apps/" + this.props.courseId + "/progress/average"
                  }
                >
                  Score details
                </a>
          ...
          <BaseStatCard
            ...
            mainValue={
              this.state.courseData.getIn([
                "learners_enrolled",
                "current_month",
              ]) - this.state.courseData.get("tma_learners_passed")
            }
            compareToPrevious={false}
            enableHistory={false}
